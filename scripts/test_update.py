...
class ManifestValidation(unittest.TestCase):
    def setUp(self):
        self.manifest = {'version':'1.2.3','tag':'personal-v1.2.3','commit':'a'*40,'repository':'sasha00123/zed','cask':'sasha-edit','app_name':'SashaEdit','bundle_id':'io.sasha00123.SashaEdit','source_asset':'sasha-edit-1.2.3-source.tar.gz','assets':[
            {'architecture':arch,'asset':f'sasha-edit-1.2.3-macos-{arch}.zip','sha256':'b'*64} for arch in ('arm64','x86_64')]}
    ...
    def test_rejects_missing_source(self):
        ...
        with self.assertRaises(ValueError):render('zed',self.release,self.manifest)
    def test_rejects_single_architecture(self):
